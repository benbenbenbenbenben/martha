...
describe("Martha!", () => {
    ...
    describe("basic types", () => {
        it("should parse type name", () => {
            expect(Tibu.parse(`Foo`)(Mod.typedef_name)).to.deep.eq([{name:["Foo"]}]);
        });
        it("should parse a base type name", () => {
            expect(Tibu.parse(`Bar`)(Mod.typedef_basetype)).to.deep.eq([{basetype:["Bar"]}]);
        });
        it("should parse a type member", () => {
            expect(Tibu.parse(`Type: Member`)(Mod.typedef_member)).to.deep.eq([{members:[{type:"Type",name:"Member"}]}]);
        });
        it("should parse a type with a base type", () => {
            expect(Tibu.parse(`type: Foo is: Bar`)(Mod.typedef)).to.deep.eq([{types:[{name:"Foo",basetype:"Bar"}]}]);
        });
        it("should parse a basic type", () => {
            expect(Tibu.parse(`type: Foo`)(Mod.typedef)).to.deep.eq([ { types: [ { name: "Foo" } ] } ]);
        });
